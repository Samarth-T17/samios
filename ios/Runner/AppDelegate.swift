...
@objc class AppDelegate: FlutterAppDelegate {
  override func application(
    ...
    let channel = FlutterMethodChannel(name: "coreml_channel", binaryMessenger: controller.binaryMessenger)

    channel.setMethodCallHandler({
      (call: FlutterMethodCall, result: @escaping FlutterResult) -> Void in
      if call.method == "runModels" {
          let output = runModels()
          result(output)
      } else {
